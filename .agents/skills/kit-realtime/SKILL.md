...
Effect owns connection lifecycle, retries, configuration, and typed transport failures. Keep simulation rate, snapshot rate, and render rate explicit. The loop consumes client intent at a tick boundary rather than on message arrival, session opens and closes are ledger events, and `snapshot()` is canonically ordered; those three are what let `apps/server/src/replay.ts` reproduce a room, so treat them as a contract and not as style. Do not persist per-tick state or route transforms through React state.
